...
<div class="card">
    ...
    <div class="card-body"> 
        <form class="form form-horizontal {{ (isAjaxRequest())?'ajax-form':'' }} validate-form" method="post" action="{{ admin_url('tools/editFabric/'.safe_b64encode($fabric->fabric_id)) }}" enctype="multipart/form-data">
            {{ csrf_field() }}
            ...
            <div class="form-body">
               ...
               <div class="form-group row">
                    ...
                    <div class="col-md-6">
                        <select id="tool_id" name="tool_id" class="custom-select form-control">
                            @if($tools)
                                ...
                                <option value="{{ $tool->tool_id }}" {{ (isset($fabric->tool_id) && $fabric->tool_id == $tool->tool_id)?"SELECTED":"" }} >{{ $tool->tool_name }}</option>    
                                @endforeach
                            @endif
                        </select>    
                    </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-6">
                        <input type="text" id="fabric_name" name="fabric_name" class="form-control validate[required]" placeholder="Fabric Name" value="{{ old('fabric_name', $fabric->fabric_name) }}" >
                    </div>
                </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-6">
                        <input type="text" id="fabric_code" name="fabric_code" class="form-control" placeholder="Fabric Code" value="{{ old('fabric_code', $fabric->fabric_code) }}" >
                    </div>
                </div>               
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-6">
                      <input type="hidden" id="uploaded_fabric_image" name="uploaded_fabric_image" value="{{ $fabric->image }}">
                      <input class="form-control-file" type="file" id="fabric_image" name="fabric_image">
                      <img id="fabric-image-view" src="{!! image_url('tools/fabrics/'.$fabric->image) !!}" width ="75" onerror="this.src='{!! image_url('no_image/item-no-image.jpg') !!}'" />
                    </div>
                    </div>
                </div>                            
                ...
                <div class="form-group row">
                  ...
                  <div class="d-inline-block custom-control custom-radio mr-1">
                    <input type="radio" class="custom-control-input bg-primary" name="status" id="status_active" value="ACTIVE" {{ $fabric->status == "ACTIVE" ? 'CHECKED' : '' }} >
                    <label class="custom-control-label" for="status_active">Active</label>
                  </div>
                  <div class="d-inline-block custom-control custom-radio mr-1">
                    <input type="radio" class="custom-control-input bg-danger" name="status" id="status_disabled" value="DISABLED" {{ $fabric->status == "DISABLED" ? 'CHECKED' : '' }} >
            ...
            <div class="form-actions">
                <button type="submit" name="update_general" class="btn btn-primary" data-callback="callBackEditFabric">
                    Save
                </button>
            </div>
            </div>
...
<script type="text/javascript">
  function callBackEditFabric(data)
  {
     if(data.status == "SUCCESS")
     {                
        $('#fabric-image-view').attr('src',data.image_url);
        $('#uploaded_fabric_image').val(data.image);
